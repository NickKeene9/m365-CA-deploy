<#
.SYNOPSIS
    CA-16-Block-AllUsers-UnmanagedDevice-SensitiveApps

.DESCRIPTION
    Blocks access to SharePoint, Exchange, and Teams from devices that are neither
    compliant nor hybrid joined. Good stepping stone before full CA-14 enforcement.

.NOTES
    Policy        : CA-16-Block-AllUsers-UnmanagedDevice-SensitiveApps
    License       : Entra ID P1 + Intune
    Default state : Report-only (enabledForReportingButNotEnforced). Pass -Enforce to deploy enabled.
    Gotchas:
        - App IDs are well-known fixed values, consistent across all tenants.
        - The device filter uses -ne to target unmanaged devices. Filter syntax is case sensitive.

.EXAMPLE
    .\CA-16-Block-AllUsers-UnmanagedDevice-SensitiveApps.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-16-Block-AllUsers-UnmanagedDevice-SensitiveApps.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-16-Block-AllUsers-UnmanagedDevice-SensitiveApps.ps1 -WhatIf
...
$PolicyName = 'CA-16-Block-AllUsers-UnmanagedDevice-SensitiveApps'
