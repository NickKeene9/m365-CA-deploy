<#
.SYNOPSIS
    CA-19-Session-AllUsers-SignInFrequency

.DESCRIPTION
    Persistent tokens are heavily abused after harvest. This session control limits how
    long a stolen token stays useful by forcing reauthentication every 8 hours.

.NOTES
    Policy        : CA-19-Session-AllUsers-SignInFrequency
    License       : Entra ID P1
    Default state : Report-only (enabledForReportingButNotEnforced). Pass -Enforce to deploy enabled.
    Gotchas:
        - Consider a separate scoped policy at 1 hour for SharePoint/Exchange/Teams if the client's risk profile warrants it.
        - Pair with CA-20 for full session hardening.

.EXAMPLE
    .\CA-19-Session-AllUsers-SignInFrequency.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-19-Session-AllUsers-SignInFrequency.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-19-Session-AllUsers-SignInFrequency.ps1 -WhatIf
...
$PolicyName = 'CA-19-Session-AllUsers-SignInFrequency'
...
$Policy = @{
    ...
    sessionControls = @{ signInFrequency = @{ isEnabled = $true; type = 'hours'; value = 8 } }
}
