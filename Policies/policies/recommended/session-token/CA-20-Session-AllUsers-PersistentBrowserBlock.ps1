<#
.SYNOPSIS
    CA-20-Session-AllUsers-PersistentBrowserBlock

.DESCRIPTION
    Prevents 'stay signed in' persistence so browser sessions end when the browser
    closes, shrinking the window for cookie and token theft.

.NOTES
    Policy        : CA-20-Session-AllUsers-PersistentBrowserBlock
    License       : Entra ID P1
    Default state : Report-only (enabledForReportingButNotEnforced). Pass -Enforce to deploy enabled.
    Gotchas:
        - Users will need to sign in again after closing the browser. Set expectations with the client before deploying.
        - Pair with CA-19.

.EXAMPLE
    .\CA-20-Session-AllUsers-PersistentBrowserBlock.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-20-Session-AllUsers-PersistentBrowserBlock.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-20-Session-AllUsers-PersistentBrowserBlock.ps1 -WhatIf
...
$PolicyName = 'CA-20-Session-AllUsers-PersistentBrowserBlock'
