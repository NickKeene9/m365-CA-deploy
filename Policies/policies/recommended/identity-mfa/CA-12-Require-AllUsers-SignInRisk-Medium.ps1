<#
.SYNOPSIS
    CA-12-Require-AllUsers-SignInRisk-Medium

.DESCRIPTION
    Challenges medium risk sign-ins with MFA instead of blocking outright, pairing with
    CA-09's hard block on high risk.

.NOTES
    Policy        : CA-12-Require-AllUsers-SignInRisk-Medium
    ...
    Gotchas:
        - Sign-in risk condition requires Entra ID P2. Document as a gap if unlicensed.

.EXAMPLE
    .\CA-12-Require-AllUsers-SignInRisk-Medium.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-12-Require-AllUsers-SignInRisk-Medium.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-12-Require-AllUsers-SignInRisk-Medium.ps1 -WhatIf
...
$PolicyName = 'CA-12-Require-AllUsers-SignInRisk-Medium'
