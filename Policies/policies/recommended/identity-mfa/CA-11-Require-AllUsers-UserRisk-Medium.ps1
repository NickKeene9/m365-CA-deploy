<#
.SYNOPSIS
    CA-11-Require-AllUsers-UserRisk-Medium

.DESCRIPTION
    Same remediation flow as CA-10 but triggered at medium user risk.

.NOTES
    Policy        : CA-11-Require-AllUsers-UserRisk-Medium
    ...
    Gotchas:
        - Same SSPR dependency as CA-10. passwordChange fails for users without SSPR configured.

.EXAMPLE
    .\CA-11-Require-AllUsers-UserRisk-Medium.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-11-Require-AllUsers-UserRisk-Medium.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-11-Require-AllUsers-UserRisk-Medium.ps1 -WhatIf
...
$PolicyName = 'CA-11-Require-AllUsers-UserRisk-Medium'
