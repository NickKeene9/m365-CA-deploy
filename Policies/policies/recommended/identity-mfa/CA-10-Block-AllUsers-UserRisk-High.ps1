<#
.SYNOPSIS
    CA-10-Block-AllUsers-UserRisk-High

.DESCRIPTION
    When Identity Protection flags a user as high risk, this policy requires MFA and a
    secure password change to remediate the account.

.NOTES
    Policy        : CA-10-Block-AllUsers-UserRisk-High
    ...
    Gotchas:
        - passwordChange requires SSPR enabled for the targeted users. Confirm before deploying or users can get stuck mid-remediation with no way to complete the flow.
        - The grant uses operator AND with both mfa and passwordChange. Graph rejects passwordChange without that pairing.

.EXAMPLE
    .\CA-10-Block-AllUsers-UserRisk-High.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-10-Block-AllUsers-UserRisk-High.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-10-Block-AllUsers-UserRisk-High.ps1 -WhatIf
...
$PolicyName = 'CA-10-Block-AllUsers-UserRisk-High'
...
if ($PSCmdlet.ShouldProcess($PolicyName, "Create Conditional Access policy in state '$State'")) {
    ...
    Write-Host ''
    Write-Host 'Reminder: confirm SSPR is enabled for targeted users before this policy moves to enforced.' -ForegroundColor Yellow
}
#endregion
