<#
.SYNOPSIS
    CA-01-Block-AllUsers-LegacyAuthentication

.DESCRIPTION
    Blocks all legacy authentication (Exchange ActiveSync and Other clients) for every
    user in the tenant. Legacy protocols cannot enforce MFA and are the single most
    common entry point in the breaches this framework remediates.

.NOTES
    Policy        : CA-01-Block-AllUsers-LegacyAuthentication
    License       : Entra ID P1
    Default state : Report-only (enabledForReportingButNotEnforced). Pass -Enforce to deploy enabled.
    Gotchas:
        - The client apps condition must select BOTH 'exchangeActiveSync' and 'other'.
        - Missing either one leaves a gap.
        - Devices that genuinely need legacy auth go in CA-EX-AllowLegacyAuthentication.
        - There is no separate allow policy. The exclusion group is the entire exception mechanism.

.EXAMPLE
    .\CA-01-Block-AllUsers-LegacyAuthentication.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-01-Block-AllUsers-LegacyAuthentication.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-01-Block-AllUsers-LegacyAuthentication.ps1 -WhatIf
...
$LegacyAuthExGroupId = Get-CAGroupId -DisplayName 'CA-EX-AllowLegacyAuthentication'
#endregion
...
$PolicyName = 'CA-01-Block-AllUsers-LegacyAuthentication'
...
$Policy = @{
    ...
    conditions  = @{
        users = @{
            includeUsers  = @('All')
            excludeGroups = @($BreakGlassGroupId, $LegacyAuthExGroupId)
        }
