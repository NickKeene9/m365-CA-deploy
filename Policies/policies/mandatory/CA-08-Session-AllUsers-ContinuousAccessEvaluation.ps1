<#
.SYNOPSIS
    CA-08-Session-AllUsers-ContinuousAccessEvaluation

.DESCRIPTION
    *** DEPLOYS ENABLED BY DEFAULT. NOT REPORT-ONLY. NO -ENFORCE SWITCH. ***

    Unlike the rest of the framework, this policy deploys enabled immediately.
    Report-only does not actually revoke anything, so it provides no safety margin for
    a session control like this. CAE in strict location mode revokes tokens within
    minutes of a policy or location change rather than waiting for token expiry.

.NOTES
    Policy        : CA-08-Session-AllUsers-ContinuousAccessEvaluation
    License       : Entra ID P1
    Default state : ENABLED. Deploys enforced immediately by design. No -Enforce switch.
    Gotchas:
        - Do not assume this script behaves like the others. It deploys enabled, on purpose.
        - Also enable tenant-wide CAE separately under Entra > Security > Continuous access evaluation. This policy alone does not flip that setting.
        - Strict location mode requires accurate named locations. Incomplete corporate IP definitions cause spurious token revocations.

.EXAMPLE
    .\CA-08-Session-AllUsers-ContinuousAccessEvaluation.ps1
    Deploys the policy ENABLED. This script intentionally has no -Enforce switch.

.EXAMPLE
    .\CA-08-Session-AllUsers-ContinuousAccessEvaluation.ps1 -WhatIf
...
$PolicyName = 'CA-08-Session-AllUsers-ContinuousAccessEvaluation'
...
$Policy = @{
    ...
    sessionControls = @{ continuousAccessEvaluation = @{ mode = 'strictLocation' } }
}
...
if ($PSCmdlet.ShouldProcess($PolicyName, "Create Conditional Access policy in state '$State'")) {
    ...
    Write-Host ''
    Write-Host 'Reminder: enable tenant-wide CAE under Entra > Security > Continuous access evaluation. This policy does not do that for you.' -ForegroundColor Yellow
}
#endregion
