<#
.SYNOPSIS
    CA-02-Require-AllUsers-MFA

.DESCRIPTION
    The broadest baseline policy in the framework. Every other MFA policy refines this
    one. Requires multifactor authentication for all users against all cloud apps.

.NOTES
    Policy        : CA-02-Require-AllUsers-MFA
    License       : Entra ID P1
    Default state : Report-only (enabledForReportingButNotEnforced). Pass -Enforce to deploy enabled.
    Gotchas:
        - Disable Security Defaults before enabling this policy. They conflict.
        - Service accounts that cannot perform MFA go in CA-EX-ServiceAccounts with documented justification.

.EXAMPLE
    .\CA-02-Require-AllUsers-MFA.ps1
    Deploys the policy in report-only mode (enabledForReportingButNotEnforced).

.EXAMPLE
    .\CA-02-Require-AllUsers-MFA.ps1 -Enforce
    Deploys the policy enabled.

.EXAMPLE
    .\CA-02-Require-AllUsers-MFA.ps1 -WhatIf
...
$PolicyName = 'CA-02-Require-AllUsers-MFA'
...
if ($PSCmdlet.ShouldProcess($PolicyName, "Create Conditional Access policy in state '$State'")) {
    ...
    Write-Host ''
    Write-Host 'Reminder: confirm Security Defaults are disabled in this tenant before moving this policy to enforced.' -ForegroundColor Yellow
}
#endregion
